...
- 기본값: `kna-wiki-a1b2` (예시값. Billing Export 를 BigQuery 로 내보내도록 설정한 내 프로젝트 ID 로 바꾼다)
...
```bash
BILLING_PROJECT=${1:-kna-wiki-a1b2}
...
| `/gcp-billing-projects` | 계정별 프로젝트 (이 레포에 포함되지 않음) |
| `/gcp-usage` | API 사용량 조회 (이 레포에 포함되지 않음) |
